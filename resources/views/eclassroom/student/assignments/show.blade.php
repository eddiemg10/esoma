            ...
            <div class="flex flex-col gap-y-5 mt-10 px-10 w-[100%]" id="quiz-content">
                <form action="" class="w-full flex flex-col items-start">
                    @foreach ($quiz as $i =>$que)
                        <p class="pl-5 text-lg bg-zinc-100 rounded-xl w-full text-left py-5"><span class="font-black text-xl">{{($i+1).". "}}</span>{{$que['question']}}</p>
                        ...
                        <div class="mb-10 py-5 w-[100%]">
                            ...
                            @foreach($que['choices'] as $i=>$choice)
                               ...
                               <p class="choice bg-sky-100 hover:cursor-default hover:bg-sky-200 transition py-4 my-3 text-left pl-10" data-question={{$que['id']}} data-choice={{rawurlencode($choice)}}> <span class="font-black pr-2">{{$alphabet[$i].". "}}</span>{{$choice}}</p> 
                            @endforeach
                        </div>
                    ...
                    <div class="w-full flex flex-col items-center justify-center">
                        ...
                        <button id="submit" class="bg-green-500 w-[20%] py-2 px-8 mt-10 rounded-md shadow-md text-white">Submit</button>
                    </div>
                    @endif
                </form>
        ...
        $(document).ready(function(){
            ...
            $("#submit").click(function(e){
                ...
                function(data, status){
                    ...
                    if(status==='success'){
                        $.get('/submitnotification', function(data, status){
                            $("#loading").hide();
