...
<span class="text-slate-400 hover:cursor-pointer">
    ...
    <a href={{"/elib/".$selectedclass->id}}
        class="hover:text-blue-900">{{$selectedclass->classlevel_name}}</a>
...
<div class="flex flex-wrap w-[100%] gap-10 justify-center">
    ...
    <div
        class="h-[200px] w-[300px] rounded-xl bg-white shadow-md md:w-[300px] transition ease-in-out hover:shadow-lg hover:">
        <div class="w-100 flex justify-center items-center h-16 rounded-tr-md rounded-tl-md">
            <a href="{{url('elib/'.$selectedclass->classlevel_name.'/'.$subject->subject_name)}}"
                class="text-center bg-yellow-500 text-white w-[100%] py-5 rounded-tr-md rounded-tl-md shadow-md">{{$subject->subject_name}}</a>
        </div>
        ...
        @if($counter == 0)
        <div class="flex mt-10  text-xl w-full justify-center py-2 text-zinc-400 transition assignment">
